...
public class FileServiceImpl implements FileService{
	...
	public void getList(ModelAndView mView, int pageNum) {
		...
		//전체 row 의 갯수를 읽어온다. | FileDao.getInstance() = dao
		...
		//1. FileDto 객체를 전달해서 파일 목록을 불러온다 | FileDao.getInstance() = dao
...
//		request.setAttribute("totalPageCount", totalPageCount);
		...
		// request에 담는 대신 modelAndView에 담음
	...
	public void insert(HttpServletRequest request, FileDto dto) {
		...
		try{
			//upload 폴더에 파일을 저장한다.
			mFile.transferTo(new File(filePath+saveFileName)); //실제 파일이 저장되는 시점
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public void delete(HttpServletRequest request, int num) {
		...
		//삭제할 파일의 정보
		FileDto dto=dao.getData(num);
		if(!id.equals(dto.getWriter())) { //문자열은 비교연산자로 비교하면 안됨!!!
			//금지된 요청이라는 응답을 한다
